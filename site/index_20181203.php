...
<!-- Modal -->
    ...
    <div id="finestraSecundaria" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style='width:75%;z-index:1500;max-height:800px; margin: 30px auto'>
        <div class="modal-dialog">
          <div style='box-shadow: rgba(0,0,0,0.6) -20px 40px 40px 0;'>          
            <div class="modal-header" id='capFinestraSecundaria' style="background-color:#3E394A;color:#ffffff;">
...
if (tp=='centres'){
	...
	row_cap[0]=['id','codi_centre','Latitud','Longitud','Codi_ST','Nom_programa','curs','Municipi','Naturalesa','Nom','TipusCentre','Logo','Web']
	row_cap[1]=['id','codi_centre','Latitud','Longitud','selFiltre','selFiltre','selFiltre','Municipi','selFiltre','Nom','selFiltre','Logo','Web'];	
	row_cap[2]=['taula:visu','codi_centre','Latitud','Longitud','taula:visu','taula:visu','taula:visu','taula:visu','taula:visu','taula:visu','taula:visu','Logo','Web'];	
}
...
var numPag=1;
var oms;
var opacitatFonsPoligon=0.2;
var opacitatVoraPoligon=0.2;
...
function carregaDades(opc,valors){
  console.log(valors);
	if (opc=="recollidaDades" && tp=='centres'){
		opc='recollidaCentres';
	}
  /*
  if (valors["Nom_SE"]!=""){
      var nomSE=valors["Nom_SE"];
  }
  //*/
  values="opc="+opc;
  ...
  $.ajax({
    ...
    success:  function (resposta) {
      ...
      case 'recollidaPrograma':
        row_prg=[];
        row_prg=resposta;
        val=row_prg[0]["id_programa"]+","+"SE del Priorat";
        console.log(valors);
        carregaDades("recollidaCentresPrograma",valors);       
        break;

      case 'recollidaCentresPrograma':
        row=[];
        row=resposta;
        onVisuPrograma(row_prg,row);       
        break;
...
function presentaEst(dades) {
  console.log(dades);
  $('#imgtemp').css("visibility","visible"); 
    txtDiv ="<table id='tblDadesEst' class='display' style='width:75%; display:block; margin: 0px auto;' >"
        txtDiv +="<tr>";
          txtDiv +="<th>id</td>";
  ...
  //poligons
	if(row_poligons!=null) {
      ...
      coefOpacitat=(valMax-valMin)/5;
            ...
            opacitatFonsPoligon=(row_poligons[p]["percent"]/100)*2.5;
            //console.log(row_poligons[p]["percent"]+"---"+opacitatFonsPoligon);
						poligons[p] = L.polygon(geo[p], {
						//fillOpacity: opacitatFonsPoligon,
            fillOpacity: opacitatFonsPoligon,            
						color: color,
						strokeWidth: gruixVoraPoligon,
						strokeOpacity: opacitatVoraPoligon,
            pane: 'poligons',
            interactive: true
					});
          infoPoligon="<div style='font-size:1.7rem;padding:5px'><strong>"+row_poligons[p]["Nom_SE"]+"</b></div>";
            ...
            infoPoligon +="</table>";                      
            infoPoligon +="<div style='padding: 2px 5px; text-align:center'><a href='#' id=\""+row_poligons[p]["Nom_SE"]+"\" onClick='carregaDades(\"recollidaEst\",this.id)'>Estadístiques</a></div>";                                  
          infoPoligon +="</div>";          
...
function onVisuDescripcio(dades){
  ...
  var msgTitol="<div style='width:15%;float: left;margin-top: -20px;cursor: default;border-right:  3px solid;padding-right:10px;'> <img src='"+dades[0]["Simbol"]+"' style='max-height: 125px'></div>";      
     msgTitol +="<div style='width:75%; float: right'>"+dades[0]["Nom_programa"]+"</div>";
  ...
  var divMsg="<div>";
    ...
    divMsg +="<div style='float:left;width:50%'>";
      divMsg +="<p class='subTitol'>";
        divMsg +="<br><b>Centre: <b>"+dades[0]['Nom'];
        divMsg +="<br><b>Titularitat: </b>"+dades[0]['Naturalesa'];
        divMsg +="<br><b>Tipus de centre: </b>"+dades[0]['TipusCentre'];        
        divMsg +="<br><b>Municipi: </b>"+dades[0]['Municipi'];
        divMsg +="<br><b>Servei Territorial: </b>"+dades[0]['Codi_ST']+"";
        divMsg +="<br><b>Programa: <b>"+dades[0]['Nom_programa'];         
        divMsg +="<br><b>Cursos del centre en el programa: <b>"+dades[0]['Cursos'];                 
        divMsg +="<br><b>Àmbits curriculars: <b><p style='margin-left:25px'>"+dades[0]['Ambits_curriculars']+"</p>";                 
        divMsg +="<br><b>Àmbits d'innovació: <b><p style='margin-left:25px'>"+dades[0]['Ambits_innovacio']+"</p>";                                 
        divMsg +="<br><b>Descripció: <b><p style='margin-left:25px'>"+dades[0]['Descripcio']+"</p>";                 
    divMsg +='</div>';
  divMsg +='</div>';    
  ...
  var msgPeu="";
    if (dades[0]['Enllac']!="" && dades[0]['Enllac']!=null){
      ...
      msgPeu +="<a href='#' class='url' style='cursor: default; margin: 5px'><button class='btn' aria-hidden='true' style='color:grey'>Fitxa del projecte</button></a>";      
    }
    ...
    if (dades[0]['Web']!="" && dades[0]['Web']!=null){
      ...
      msgPeu +="<a href='#' class='url' style='cursor: default; margin: 5px'><button class='btn' aria-hidden='true' style='color:grey'>Web del centre</button></a>";         
    }
    msgPeu +="<button class='btn' data-dismiss='modal' aria-hidden='true' style='margin: 5px' >Tancar</button>";
...
function onVisuEst(dades){
  var msgTitol="<div style='width:100%; float: right'>"+dades[0]["Nom_SE"]+"</div>";
    divMsg ="<table id='tblInfoEst' style='width:100%; padding: 10px;margin:3px'>";
        divMsg +="<tr>";
          divMsg +="<th></th>";                    
...
function onVisuPrograma(dades,row_cen){
  console.log(dades+"\n"+row_cen);
  ...
  var divMsg ="<div>";
          divMsg +="<div><b>Descripció: </b></div>";                    
          divMsg +="<p>"+dades[0]["Descripcio"]+"</p>";
          divMsg +="<div><b>Àmbits curriculars: </b><p>"+dades[0]["Ambits_curriculars"]+"</p>";    
          divMsg +="<div><b>Àmbits d'innovació: </b><p>"+dades[0]["Ambits_innovacio"]+"</p>";    
        divMsg +='</div>';      
        
        divMsg +="<h3>Relació de centres</h3>";
        divMsg +="<table id='tblInfoEst' style='width:100%; padding: 10px;margin:3px'>";
          divMsg +="<tr>";
            divMsg +="<th></th>";                    
            divMsg +="<th>Centre</th>";
            ...
            divMsg +="<th>Cursos<br>programa<br>innovacio</th>";            
          divMsg +='<tr>';          
        if (row_cen!=null){
          for (f=0;f<row_cen.length;f++){
            divMsg +="<tr>";
              if (row_cen[f]["Web"]=="") {
                icoWeb="nodes_grey.png";
                divMsg +="<td><img src='imatges/"+icoWeb+"' style='margin: 5px; width:30px'/></td>";                    
              } else {
                icoWeb="nodes.png";              
                divMsg +="<td><a href='"+row_cen[f]["Web"]+"' target='_blank'><img src='imatges/"+icoWeb+"' style='margin: 5px; width:30px'/></a></td>";                    
              }
